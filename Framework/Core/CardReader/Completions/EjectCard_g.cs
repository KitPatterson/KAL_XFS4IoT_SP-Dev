...
namespace XFS4IoT.CardReader.Completions
{
    ...
    public sealed class EjectCardCompletion : Completion<EjectCardCompletion.PayloadData>
    {
        ...
        public sealed class PayloadData : MessagePayload
        {
            ...
            /// <summary>
            ///Specifies the error code if applicable. The following values are possible:* ```mediaJam``` - The card is jammed. Operator intervention is required. A possible scenario is also  when an attempt to retain the card was made during attempts to eject it. The retain bin is full; no  more cards can be retained. The current card is still in the device.* ```shutterFail``` - The open of the shutter failed due to manipulation or hardware error. Operator  intervention is required.* ```noMedia``` - No card is present.* ```mediaRetained``` - The card has been retained during attempts to eject it. The device is clear  and can be used.
            /// </summary>
