...
namespace XFS4IoT.Printer.Events
{
    ...
    public sealed class MediaDetectedEvent : Event<MediaDetectedEvent.PayloadData>
    {
        ...
        public sealed class PayloadData : MessagePayloadBase
        {
            ...
            /// <summary>
            ///Specifies the media position after the reset operation, as one of the following values:* ```retracted``` - The media was retracted during the reset operation.* ```present``` - The media is in the print position or on the stacker.* ```entering``` - The media is in the exit slot.* ```jammed``` - The media is jammed in the device.* ```unknown``` - The media is in an unknown position.* ```expelled``` - The media was expelled during the reset operation.
            /// </summary>
            [DataMember(Name = "position")] 
            public PositionEnum? Position { get; private set; }
            /// <summary>
            ///Number of the retract bin the media was retracted to. This number has to be between one and the [number of bins](#common.capabilities.completion.properties.printer.retractbins) supported by this device.It is only relevant if [position](#printer.mediadetectedevent.event.properties.position) is *retracted*.
            /// </summary>
